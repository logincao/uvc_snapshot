...
Java_com_benjaminwan_ocrlibrary_OcrEngine_setProvider(JNIEnv *env,jstring provider){
    std::string providertype=jstringToChar(env, provider);
#ifdef ORT_BUILD_WITH_ONNXRUNTIME
    if (providertype=="OnnxRuntime") {
        ocrLite->setProvider(OcrProvider::BackendType::ONNX);
    }
#endif
        
#ifdef ORT_BUILD_WITH_OPENVINO
    if (providertype=="OpenVINO") {
        ocrLite->setProvider(OcrProvider::BackendType::OPENVINO);
    }
#endif

#ifdef ORT_BUILD_WITH_NCNN
    if (providertype=="Ncnn") {
        ocrLite->setProvider(OcrProvider::BackendType::NCNN);
    }
#endif

#ifdef ORT_BUILD_WITH_MNN
    if (providertype=="MNN") {
        ocrLite->setProvider(OcrProvider::BackendType::MNN);
    }
#endif
