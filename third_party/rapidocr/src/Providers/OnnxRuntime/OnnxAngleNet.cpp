...
OnnxAngleNet::~OnnxAngleNet() {
    delete session;
}

void OnnxAngleNet::setNumThread(int numOfThread) {
...
void OnnxAngleNet::initModel(const std::string &pathStr) {
#ifdef _WIN32
    std::wstring anglePath = strToWstr(pathStr+".onnx");
...
Angle OnnxAngleNet::getAngle(cv::Mat &src) {
...
std::vector<Angle> OnnxAngleNet::getAngles(std::vector<cv::Mat> &partImgs, const char *path, const char *imgName, bool doAngle, bool mostAngle) {
...
std::vector<Ort::AllocatedStringPtr> OnnxAngleNet::getInputNames(Ort::Session *session) {
...
std::vector<Ort::AllocatedStringPtr> OnnxAngleNet::getOutputNames(Ort::Session *session) {
    Ort::AllocatorWithDefaultOptions allocator;
