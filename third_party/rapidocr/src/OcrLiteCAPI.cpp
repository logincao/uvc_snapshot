...
OcrInit(const char* szDetModel, const char* szClsModel,
        const char* szRecModel, const char* szKeyPath,
        int nThreads, const char* provider) {
    try {
        OcrLiteImpl* impl =  new OcrLiteImpl();
        ...
        if (provider) {
            if (std::strcmp(provider, "Paddle") == 0 && kPaddle) {
                impl->setProvider(BackendType::PADDLE);
            }
            else if (std::strcmp(provider, "OnnxRuntime") == 0 && kOnnxRuntime) {
                impl->setProvider(BackendType::ONNX);
            }
            else if (std::strcmp(provider, "Ncnn") == 0 && kNcnn) {
                impl->setProvider(BackendType::NCNN);
            }
            else if (std::strcmp(provider, "MNN") == 0 && kMNN) {
                impl->setProvider(BackendType::MNN);
            }
            else if (std::strcmp(provider, "OpenVINO") == 0 && kOpenVINO) {
                impl->setProvider(BackendType::OPENVINO);
            }else{
                printf("[ERROR] Backend not registered:[%s]\n",provider);
            }
        }
